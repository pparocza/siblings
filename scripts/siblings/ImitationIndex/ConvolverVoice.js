export class ConvolverVoice
{
    constructor(siblingContext, nLayers, fundamental, scale)
    {
        ...
        // change this to just generating an array, that will then be fed to a single frequencyModulatedSine method
        for (let layer = 0; layer < nLayers; layer++)
        {
            ...
            carrierFrequencyArray.push(carrierFrequency, carrierFrequency * siblingContext.randomFloat(detuneRangeMin, detuneRangeMax));
            modulatorFrequencyArray.push(modulatorFrequency, modulatorFrequency * siblingContext.randomFloat(detuneRangeMin, detuneRangeMax));
            modulatorGainArray.push(modulationGain, modulationGain * siblingContext.randomFloat(detuneRangeMin, detuneRangeMax));
        }
