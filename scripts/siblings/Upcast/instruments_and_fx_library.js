...
function SemiOpenPipe(length){
	...
	for(var i=0; i<this.lengthMultiplierArray.length; i++){
		this.filters[i] = {filter: audioCtx.createBiquadFilter()};
		this.filters[i].filter.type = "bandpass";
		this.filters[i].filter.frequency.value = this.length*(this.lengthMultiplierArray[i]);
		this.filters[i].filter.Q.value = 30;

		this.filters[i+this.lengthMultiplierArray.length] = {inletScaler: audioCtx.createGain()};
		this.filters[i+this.lengthMultiplierArray.length].inletScaler.gain.value = this.lengthMultiplierArray[i];
